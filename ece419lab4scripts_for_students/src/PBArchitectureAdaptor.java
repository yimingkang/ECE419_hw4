...
public class PBArchitectureAdaptor {
    ...
    public static MPacket exchangeMPacket(MPacket outBound) {
    	while(true){
    		try{
    ...
    public static void pbHandler(WatchedEvent event){
    	...
    	if (correctPath && (isNodeChanged || isNodeDeleted)){
    		logger.info("PB was either removed or changed");
    		getPBPort();
    	} else{
    ...
    public static void getPBPort(){
    	while (true){
	    	try {
				byte[] trackerData = zk.getData(pbPath, pbWatcher, new Stat());
